package lk.ijse.Library_Management_System.Controller;

import javafx.animation.KeyFrame;
import javafx.animation.Timeline;
import javafx.event.ActionEvent;
import javafx.fxml.FXMLLoader;
import javafx.scene.Parent;
import javafx.scene.control.Label;
import javafx.scene.control.TextField;
import javafx.scene.layout.AnchorPane;
import javafx.scene.layout.VBox;
import javafx.util.Duration;
import lk.ijse.Library_Management_System.BO.Custom.AdminBO;
import lk.ijse.Library_Management_System.BO.Custom.BookBO;
import lk.ijse.Library_Management_System.BO.Custom.Impl.AdminBoImpl;
import lk.ijse.Library_Management_System.BO.Custom.Impl.BookBoImpl;
import lk.ijse.Library_Management_System.DTO.AdminDTO;
import lk.ijse.Library_Management_System.DTO.BookDTO;
import lk.ijse.Library_Management_System.Entity.Book;
import lk.ijse.Library_Management_System.Utill.Navigation;

import java.io.IOException;
import java.sql.SQLException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import java.util.Date;

public class StaffBookFormController {
    private static StaffBookFormController controller;
    public AnchorPane pagingPane;
    public TextField txtSearchBook;
    public Label txtTime;
    public VBox vBox;
    public AnchorPane AddPane;

    BookBO bookBO = new BookBoImpl();
    AdminBO adminBO = new AdminBoImpl();

    public StaffBookFormController(){
        controller = this;
    }
    public static StaffBookFormController getInstance(){
        return controller;
    }

    public void initialize() throws SQLException, ClassNotFoundException {
        getAllId();
        Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(1), event -> txtTime.setText(timeNow())));
        timeline.setCycleCount(Timeline.INDEFINITE);
        timeline.play();
        //save();
    }

    private void save() throws SQLException, ClassNotFoundException {
        AdminDTO adminDTO = new AdminDTO();
        adminDTO.setId(1);
        adminDTO.setName("jungkook");
        boolean isAdminSaved = adminBO.saveAdmin(adminDTO);
        if (isAdminSaved){
            System.out.println("Admin Saved");
        }
        else {
            System.out.println("admin not saved");
        }
    }

    private String timeNow() {
        SimpleDateFormat dateFormat=new SimpleDateFormat("HH:mm:ss");
        return dateFormat.format(new Date()) ;
    }

    public void btnAddBookOnAction(ActionEvent actionEvent) throws IOException {
        Navigation.popupPane("StaffBookAddForm.fxml");
        //System.out.println("button clicked");
    }
    public void getAllId() throws SQLException, ClassNotFoundException {
        ArrayList<String> list = null;
        list = bookBO.getAllBookId();


        vBox.getChildren().clear();
        for(int i = 0; i< list.size(); i++){
            loadTableData(Integer.parseInt(list.get(i)));
            //System.out.println(Integer.parseInt(list.get(i)));
        }
    }
    private void loadTableData(int id) {
        System.out.println("load table data");
        try {
            FXMLLoader loader = new FXMLLoader(StaffBookFormController.class.getResource("/view/StaffBookBarForm.fxml"));
            Parent root = null;
            root = loader.load();
            StaffBookBarFormController controller = loader.getController();
            controller.setData(id);
            vBox.getChildren().add(root);
        } catch (IOException e) {
            e.printStackTrace();
        } catch (SQLException e) {
            throw new RuntimeException(e);
        } catch (ClassNotFoundException e) {
            throw new RuntimeException(e);
        }
    }
}
